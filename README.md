# 👋 Hi, I'm Mohan M

### 💻 Computer Science Engineering Student | Aspiring Software Engineer

I'm a 3rd-year CSE student passionate about building real-world software, solving problems, and learning new technologies.

* 🎓 CSE Student
* ☕ Learning **Java + Data Structures & Algorithms**
* 🗄️ Learning **SQL & Database Management**
* 🤖 Interested in **AI/ML and Generative AI**
* 🚀 Building projects and participating in **Hackathons**
* 🌱 Currently improving my **problem-solving and software development skills**

### 🛠️ Tech Stack

**Languages:**
Java • Python • SQL 

**Development:**
HTML • CSS • JavaScript • React • Next.js • Node.js

**AI/ML:**
Google Cloud • Gemini • YOLO • Computer Vision

**Tools:**
Git • GitHub • VS Code

### 🚀 Projects

🔹 **ScamShield** — AI-powered platform focused on detecting and preventing scams.

🔹 **Eco-Ideathon** — Eco-Ideathon – Rooftop Solar Detection.


### 🏆 Hackathons & Activities

* Participating in technical hackathons and innovation challenges
* Building real-world projects with team members
* Exploring AI, software engineering and emerging technologies

### 📚 Currently Learning

`Java` → `DSA` → `SQL` → `Backend Development` → `System Design`

### 🤝 Connect With Me

[LinkedIn](https://www.linkedin.com/in/mohan-m-2025cse)

---

⭐ *Always learning. Always building.*

